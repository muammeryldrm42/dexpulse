...
function renderSignalHistory(items){
  ...
  for (const it of items){
    ...
    const notes = Array.isArray(it.historyNotes) ? it.historyNotes.slice(-3).reverse() : [];
    const noteHtml = notes.length
      ? notes.map(n => `<div class="historyNote">${n.text}</div>`).join("")
      : `<div class="historyNote muted">No milestone notes yet.</div>`;
    ...
    item.innerHTML = `
      ...
      <div class="historyMeta">
        <span>Entry MC: <b>${fmtUSD(entryMc)}</b></span>
        <span>Peak MC: <b>${fmtUSD(peakMc)}</b></span>
        <span>ROI %: <b>${roiPct ? pct(roiPct) : "—"}</b></span>
        <span>ROI X: <b>${roiX ? `${roiX}x` : "—"}</b></span>
      </div>
      <div class="historyNotes">${noteHtml}</div>
    `;
